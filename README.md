...
steps:
  - label: ":rocket: Deploy"
    plugins:
      - fjall-tech/fjall-deploy#v19.0.0:
          target: my-app
```
...
| `cli-version`       | no       | `29`     | Pin the `fjall` CLI version. Defaults to this plugin's compatible major (floats across `29`.x, never crossing into the next major); `auto` derives the major from the app's pinned `@fjall/components-infrastructure`; `latest` always installs the newest published release |
...
- **The plugin ref** — `fjall-tech/fjall-deploy#v19` is a moving major tag, repointed at the latest 19.x release on each plugin republish; `#v19.0.0` pins an exact release for maximum determinism.
