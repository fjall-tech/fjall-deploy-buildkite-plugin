...
| `cli-version`       | no       | `31`     | Pin the `fjall` CLI version. Defaults to this plugin's compatible major (floats across `31`.x, never crossing into the next major); `auto` derives the major from the app's pinned `@fjall/components-infrastructure`; `latest` always installs the newest published release |
...
- **The plugin ref** — `fjall-tech/fjall-deploy#v21` is a moving major tag, repointed at the latest 21.x release on each plugin republish; `#v21.0.0` pins an exact release for maximum determinism.
